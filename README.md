...
11. [부하 테스트 및 성능 개선 (k6)](#-부하-테스트-및-성능-개선-k6)
12. [프로젝트 구조](#-프로젝트-구조)
...
- ✅ **k6 부하 테스트**: 4가지 시나리오로 성능 검증 및 병목 분석
- ✅ **성능 최적화**: 응답 시간 86% 개선, 처리량 900% 증가
- ✅ **장애 대응 체계**: P0~P3 분류, 체계적인 대응 절차 수립
...
k6를 사용한 체계적인 부하 테스트를 통해 시스템 성능을 검증하고, 병목 지점을 식별하여 **86%의 응답 시간 개선**을 달성했습니다.

### 테스트 시나리오

#### 1. 선착순 쿠폰 발급 부하 테스트
**목적**: Redis Lua Script 기반 동시성 제어 검증

- **파일**: `k6-tests/scenarios/coupon-issue.js`
- **설정**: 1,000명이 100개 한정 쿠폰 요청
- **검증**:
  - ✅ 정확히 100개만 발급 (동시성 제어 100%)
  - ✅ Race Condition 방지
  - ✅ 중복 발급 0건
...
#### 2. 주문 생성 부하 테스트
**목적**: 트랜잭션 안정성 및 처리 성능 측정

- **파일**: `k6-tests/scenarios/order-create.js`
- **설정**:
  - 1분 동안 0 → 50명으로 증가
  - 3분 동안 50명 유지
  - 1분 동안 50 → 0명으로 감소
...
#### 3. 상품 조회 부하 테스트 (스트레스 테스트)
**목적**: 높은 동시 접속 환경에서의 응답 성능 확인

- **파일**: `k6-tests/scenarios/product-list.js`
- **설정**:
  - 2분 동안 0 → 100명 증가
  - 5분 동안 100명 유지
  - 2분 동안 100 → 200명 증가 (스트레스)
  - 5분 동안 200명 유지
- **검증**:
  - ✅ 페이징 처리 성능
  - ✅ 상세 조회 성능
...
#### 4. 인기 상품 조회 부하 테스트 (스파이크 테스트)
**목적**: Redis 캐시 성능 검증 및 급격한 트래픽 증가 대응 확인

- **파일**: `k6-tests/scenarios/popular-products.js`
- **설정**:
  - 10초 만에 0 → 500명으로 급증
  - 3분 동안 500명 유지
  - 10초 만에 500 → 0명으로 감소
...
| 메트릭 | 개선 전 | Phase 1 | 개선율 |
|--------|---------|---------|--------|
| **상품 목록 P95** | 620ms | 280ms | **-55%** |
| **주문 생성 P95** | 2,850ms | 1,200ms | **-58%** |
| **인기 상품 P95** | 780ms | 320ms | **-59%** |
| **전체 처리량** | 60 TPS | 180 TPS | **+200%** |

**적용 항목**:
- ✅ N+1 쿼리 제거 (`@EntityGraph`)
- ✅ 비동기 이벤트 처리 (`AsyncConfig`)
- ✅ DB 인덱스 추가
- ✅ 커넥션 풀 최적화 (HikariCP: 10 → 50)

#### Phase 2: 캐시 적용

| 메트릭 | Phase 1 | Phase 2 | 총 개선율 |
|--------|---------|---------|------------|
| **상품 목록 P95** | 280ms | 50ms | **-92%** |
| **상품 상세 P95** | 180ms | 30ms | **-83%** |
| **인기 상품 P95** | 320ms | 45ms | **-94%** |
| **전체 처리량** | 180 TPS | 600 TPS | **+900%** |

**적용 항목**:
- ✅ Redis 캐시 설정 (`CacheConfig`)
- ✅ `@Cacheable` 적용 (ProductService)
- ✅ 캐시 히트율: 80% 이상
...
**2. 비동기 처리 미설정 (StockDeductionEventListener)**
```java
...
```
**효과**: 주문 생성 응답 시간 -58% (2,850ms → 1,200ms)

#### 🟡 Medium (중요)

**3. DB 인덱스 부족 (product_statistics)**
```sql
...
|--------|---------------|-----|-----|-----|--------|------|
| 쿠폰 발급 (동시성) | 245ms | 387ms | 512ms | - | 0% | ✅ 100개 정확 발급 |
| 주문 생성 | 680ms | 1,200ms | 2,100ms | 12 | 0.5% | ✅ 안정적 |
| 상품 조회 (개선 후) | 35ms | 50ms | 85ms | 450 | 0% | ✅ 목표 달성 |
| 인기 상품 (Redis) | 12ms | 45ms | 78ms | 850 | 0% | ✅ 탁월 |

### 장애 대응 체계

시스템 장애 발생 시 체계적인 대응을 위한 플레이북을 구축했습니다.

**장애 분류**:
- **P0 (Critical)**: 전체 서비스 다운 (복구 시간: 15분 이내)
- **P1 (High)**: 핵심 기능 장애 (복구 시간: 1시간 이내)
- **P2 (Medium)**: 일부 기능 장애 (복구 시간: 4시간 이내)
- **P3 (Low)**: 경미한 장애 (복구 시간: 24시간 이내)

**주요 장애 시나리오**:
1. 웹사이트 접속 불가 (502 Bad Gateway)
2. 응답 시간 급증 (P95 > 5초)
3. 데이터베이스 다운
4. Redis 장애
5. 선착순 쿠폰 발급 장애
...
k6 부하 테스트에 대한 자세한 내용은 아래 문서를 참조하세요:

- **테스트 계획**: [k6-tests/LOAD_TEST_PLAN.md](k6-tests/LOAD_TEST_PLAN.md) (63KB)
- **실행 가이드**: [k6-tests/TEST_EXECUTION_GUIDE.md](k6-tests/TEST_EXECUTION_GUIDE.md)
- **테스트 결과**: [k6-tests/TEST_RESULTS_SUMMARY.md](k6-tests/TEST_RESULTS_SUMMARY.md)
- **성능 분석**: [k6-tests/PERFORMANCE_ANALYSIS.md](k6-tests/PERFORMANCE_ANALYSIS.md) (23KB)
- **장애 대응**: [k6-tests/INCIDENT_RESPONSE_PLAYBOOK.md](k6-tests/INCIDENT_RESPONSE_PLAYBOOK.md) (24KB)
- **최종 요약**: [k6-tests/FINAL_SUMMARY.md](k6-tests/FINAL_SUMMARY.md)
...
├── k6-tests/                       # k6 부하 테스트 (8주차)
│   ├── README.md                          # k6 설치 및 사용 가이드
│   ├── config.js                          # 공통 설정
│   ├── LOAD_TEST_PLAN.md                  # 상세 테스트 계획서 (63KB)
│   ├── TEST_EXECUTION_GUIDE.md            # 실행 가이드
│   ├── TEST_RESULTS_SUMMARY.md            # 결과 요약
│   ├── PERFORMANCE_ANALYSIS.md            # 성능 분석 보고서 (23KB)
│   ├── INCIDENT_RESPONSE_PLAYBOOK.md      # 장애 대응 플레이북 (24KB)
│   ├── FINAL_SUMMARY.md                   # 최종 요약
│   ├── setup-test-data.sql                # 테스트 데이터 생성 (8KB)
│   └── scenarios/
│       ├── smoke-test.js                  # 사전 검증 (30초)
│       ├── coupon-issue.js                # 쿠폰 발급 테스트 (1000 VUs)
│       ├── order-create.js                # 주문 생성 테스트 (50 VUs)
│       ├── product-list.js                # 상품 조회 테스트 (200 VUs)
│       ├── popular-products.js            # 인기 상품 테스트 (500 VUs)
│       └── quick-load-test.js             # 빠른 부하 테스트 (100 VUs)
├── performance-improvements/       # 성능 개선 코드 (8주차)
│   ├── AsyncConfig.java                   # 비동기 이벤트 처리 설정
│   ├── CacheConfig.java                   # Redis 캐시 설정
│   ├── database-indexes.sql               # DB 인덱스 추가 스크립트
│   └── application-performance.yml        # HikariCP 및 최적화 설정
├── scripts/                        # SQL 스크립트
│   └── init.sql                    # 데이터베이스 초기화
...
### 9. k6 부하 테스트 및 성능 최적화 (NEW - 8주차)
- **k6 부하 테스트**: 4가지 시나리오 (쿠폰 발급, 주문 생성, 상품 조회, 인기상품)
- **성능 병목 분석**: N+1 쿼리, 비동기 미처리, DB 인덱스 부족, 커넥션 풀 부족
- **데이터 기반 개선**: 정량적 성능 측정 → 병목 식별 → 개선 → 검증
- **코드 레벨 최적화**: `@EntityGraph`, AsyncConfig, DB 인덱스, HikariCP 튜닝
- **캐시 전략**: Redis 캐시 적용으로 응답 시간 86-94% 개선
- **장애 대응 체계**: P0~P3 분류, 6가지 시나리오별 대응 절차, Post-Mortem
- **성능 개선 효과**: 전체 처리량 900% 증가, 응답 시간 86% 단축
